...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            logger.exception(e)
            fek = await message.reply_photo(photo="https://telegra.ph/file/30b027475c1b565ae1db2.jpg", caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(600)
            await fek.delete()
            await msg.delete()
    else:
        fuk = await message.reply_photo(photo="https://telegra.ph/file/d2521ad291775cedcb74f.jpg", caption=cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(600)
        await fuk.delete()
        await msg.delete()
    if spoll:
...
async def advantage_spell_chok(msg):
    ...
    btn.append([InlineKeyboardButton(text="⚠️ Close", callback_data=f'spolling#{user}#close_spellcheck')])
    m = await msg.reply(f"Hey, {msg.from_user.mention}!\n<b>I couldn't find anything related to that\nDid you mean any one of these?\n\n🔴ඔයාට අවශ්‍ය චිත්‍රපටයේ නම තිබෙන Button එක Click කර චිත්‍රපටය ලබාගන්න.</b> ", reply_markup=InlineKeyboardMarkup(btn))
